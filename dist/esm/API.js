...
var GitHubCheckRunState;

(function (GitHubCheckRunState) {
  GitHubCheckRunState["Queued"] = "queued";
  GitHubCheckRunState["In_Progress"] = "in_progress";
  GitHubCheckRunState["Completed"] = "completed";
})(GitHubCheckRunState || (GitHubCheckRunState = {}));
...
class API {
  ...
  async getCheckRuns(collectionName, slug) {
    const contentKey = this.generateContentKey(collectionName, slug);
    const branch = (0, _netlifyCmsLibUtil.branchFromContentKey)(contentKey);
    const pullRequest = await this.getBranchPullRequest(branch);
    const sha = pullRequest.head.sha;
    const resp = await this.request(`${this.originRepoURL}/commits/${sha}/check-runs`);
    return resp.check_runs.map(s => {
      var _s$output$summary$mat;

      return {
        context: s.name,
        target_url: (_s$output$summary$mat = s.output.summary.match(/Preview URL:<\/strong><\/td><td>\n<a href='(.*)'>/)) === null || _s$output$summary$mat === void 0 ? void 0 : _s$output$summary$mat[1],
        state: s.status === GitHubCheckRunState.Completed ? _netlifyCmsLibUtil.PreviewState.Success : _netlifyCmsLibUtil.PreviewState.Other
      };
    });
  }
